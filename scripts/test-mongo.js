...
async function runMongoTests() {
  ...
  try {
    ...
    const mongoTokenDoc = await directDb
      .collection("tokens")
      .findOne({ _id: generatedToken.id });
    assert(
      mongoTokenDoc !== null &&
        mongoTokenDoc.name === testTokenName &&
        mongoTokenDoc.token === generatedToken.token,
    ...
    assert(
      ...
      "tokenService.validateToken validates token from MongoDB-backed store",
    );
    ...
    await new Promise((r) => setTimeout(r, 150));
    const revokedDoc = await directDb
      .collection("tokens")
      .findOne({ _id: generatedToken.id });
    assert(
      revokedDoc === null,
      'Revoked token is deleted from MongoDB "tokens" collection',
    );
    ...
    assert(
      loggedActivity.id.startsWith("act_"),
      "activityService.log records new activity",
    );
    await new Promise((r) => setTimeout(r, 150));

    // Verify record in MongoDB
    const mongoActDoc = await directDb
      .collection("activities")
      .findOne({ _id: loggedActivity.id });
    assert(
      mongoActDoc !== null &&
        mongoActDoc.recipient === "201012345678" &&
        mongoActDoc.status === "SENT",
    ...
    await new Promise((r) => setTimeout(r, 150));
    const deletedDoc = await directDb
      .collection("activities")
      .findOne({ _id: loggedActivity.id });
    assert(
      deletedDoc === null,
      'Deleted activity is removed from MongoDB "activities" collection',
    );
    ...
    assert(
      adminKey.startsWith("adm_live_"),
      "adminService generates secure admin key",
    );

    const adminDoc = await directDb
      .collection("settings")
      .findOne({ _id: "admin_secret" });
    assert(
    ...
    assert(
      webhookService.getStatus().url === testWebhookUrl,
      "webhookService.setWebhookUrl sets url in memory",
    );
    await new Promise((r) => setTimeout(r, 150));

    const webhookDoc = await directDb
      .collection("settings")
      .findOne({ _id: "webhook_config" });
    assert(
    ...
    assert(
      mongoAuth.state && mongoAuth.state.creds && mongoAuth.saveCreds,
      "useMongoAuthState returns valid auth state and saveCreds",
    );

    // Test creds saving
    mongoAuth.state.creds.me = {
      id: "201012345678:1@s.whatsapp.net",
      name: "Test Bot",
    };
    ...
    assert(
      credsDoc !== null && typeof credsDoc.data === "string",
      'WhatsApp credentials saved to MongoDB collection "baileys_auth"',
    );
    ...
    const retrievedKeys = await mongoAuth.state.keys.get("session", [
      sampleKeyId,
    ]);
    assert(
    ...
    try {
      const healthRes = await fetch(
        `http://127.0.0.1:${testHttpPort}/api/health`,
      );
    ...
    assert(
      !databaseService.isConnected(),
      "databaseService.disconnect() terminates MongoDB connection cleanly",
    );
